...
load(struct mydate *t, int *len, int ignore_open_error)
{
    ...
    struct record *rec_p;
    int recs_num;
    FILE *fp = open(t->year, t->month, "r", ignore_open_error);
    int i, p = 0, line = 0;
...
modeadd(int argc_org, char *argv_org[])
{
    ...
    append(time.year, time.month, DELIMITER, date2str(&time), cat, loc, note, money);
...
modeshow(int argc, char *argv[])
{
    ...
    else
    {
        ...
        {
            parsedate(argv[1], &etime, MASK_YEAR);
            if (date2int(&stime) > date2int(&etime))
            {
                fprintf(stderr, "Error: 日付の範囲指定が間違っています\n");
                exit(EXIT_FAILURE);
            }
            if (stime.mask != etime.mask)
            {
                fprintf(stderr, 
                        "Error: 日付範囲の初めと終わりを指定する場合，"
                        "どちらも同じレベルまで指定する必要があります\n");
                exit(EXIT_FAILURE);
            }

            if (etime.mask == MASK_YMD)       incdate(&etime, MASK_DAY);
            else if (etime.mask == MASK_YM)   incdate(&etime, MASK_MONTH);
            else if (etime.mask == MASK_YEAR) incdate(&etime, MASK_YEAR);
        }
    }
